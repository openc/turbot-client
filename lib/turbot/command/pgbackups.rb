...
module Turbot::Command
  ...
  # manage backups of turbot postgresql databases
  class Pgbackups < Base
    ...
    def capture
      ...
      if backup["error_at"]
        ...
        message += "\nPlease run `turbot logs --ps pgbackups` for details."
    ...
    def destroy
      unless name = shift_argument
        error("Usage: turbot pgbackups:destroy BACKUP_ID\nMust specify BACKUP_ID to destroy.")
      end
    ...
    #Example:
    #
    #$ turbot pgbackups:transfer green teal --app example
    #
    # note that both the FROM and TO database must be accessible to the pgbackups service
    #$ turbot pgbackups:transfer DATABASE postgres://user:password@host/dbname --app example
    #
    def transfer
      db1 = shift_argument
      db2 = shift_argument
    ...
    def pgbackup_client
      ...
      error("Please add the pgbackups addon first via:\nturbot addons:add pgbackups") unless pgbackups_url
      @pgbackup_client ||= Turbot::Client::Pgbackups.new(pgbackups_url)
    end
...
Verify the status of your backup with `turbot pgbackups -a #{app}`
You can also watch progress with `turbot logs --tail --ps pgbackups -a #{app}`
      EOM
    end
    ...
    def no_backups_error!
      error("No backups. Capture one with `turbot pgbackups:capture`.")
    end
