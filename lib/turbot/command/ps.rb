...
#
class Turbot::Command::Ps < Turbot::Command::Base
  PRICES = {
    "P"  => 0.8,
    "PX" => 0.8,
  }
  ...
  # ps:dynos [QTY]
  #
  # DEPRECATED: use `turbot ps:scale dynos=N`
  #
  ...
  def dynos
    ...
    display("~ `turbot ps:dynos QTY` has been deprecated and replaced with `turbot ps:scale dynos=QTY`")
    ...
    if dynos
      ...
      if app_data["stack"] == "cedar"
        raise(Turbot::Command::CommandFailed, "For Cedar apps, use `turbot ps`")
      else
  ...
  # ps:workers [QTY]
  #
  # DEPRECATED: use `turbot ps:scale workers=N`
  #
  ...
  def workers
    # deprecation notice added to v2.21.3 on 03/16/12
    display("~ `turbot ps:workers QTY` has been deprecated and replaced with `turbot ps:scale workers=QTY`")
  ...
  def index
    ...
    resp = api.request(
      ...
      :path    => "/apps/#{app}/dynos",
      :headers => {
        "Accept"       => "application/vnd.turbot+json; version=3",
  ...
  #Examples:
  #
  # $ turbot ps:restart web.1
  # Restarting web.1 dyno... done
  #
  # $ turbot ps:restart web
  ...
  #Examples:
  #
  # $ turbot ps:scale web=3:2X worker+1
  ...
  def scale
    ...
    if changes.empty?
      error("Usage: turbot ps:scale DYNO1=AMOUNT1[:SIZE] [DYNO2=AMOUNT2 ...]\nMust specify DYNO and AMOUNT to scale.")
    end

    action("Scaling dynos") do
      # The V3 API supports atomic scale+resize, so we make a raw request here
      # since the turbot-api gem still only supports V2.
      resp = api.request(
        :expects => 200,
        :method  => :patch,
  ...
  def stop
    ...
    message, options = case dyno
    when NilClass
      error("Usage: turbot ps:stop DYNO\nMust specify DYNO to stop.")
    when /.+\..+/
      ps = args.first
  ...
  # Example:
  #
  # $ turbot ps:resize web=PX worker=2X
  ...
  def resize
    ...
    if changes.empty?
      message = [
          "Usage: turbot ps:resize DYNO1=1X|2X|PX [DYNO2=1X|2X|PX ...]",
