...
class Turbot::Command::Apps < Turbot::Command::Base
  ...
  # http://floating-dragon-42.turbot.com/ | git@turbot.com:floating-dragon-42.git
  #
  # $ turbot apps:create -s bamboo
  # Creating floating-dragon-42... done, stack is bamboo-mri-1.9.2
  # http://floating-dragon-42.turbotapp.com/ | git@turbot.com:floating-dragon-42.git
  #
  # # specify a name
  # $ turbot apps:create example
  # Creating example... done, stack is cedar
  # http://example.turbot.com/ | git@turbot.com:example.git
  #
  # # create a staging app
  # $ turbot apps:create example-staging --remote staging
  #
  # # create an app in the eu region
  # $ turbot apps:create --region eu
  #
  def create
    ...
    begin
      ...
      hputs("Timed Out! Run `turbot status` to check for known platform issues.")
    end

    unless options[:no_remote].is_a? FalseClass
      create_git_remote(options[:remote] || "turbot", info["git_url"])
    end
  end
  ...
  #Example:
  #
  # $ turbot apps:rename example-newname
  # http://example-newname.turbotapp.com/ | git@turbot.com:example-newname.git
  # Git remote turbot updated
  #
  def rename
    newname = shift_argument
    if newname.nil? || newname.empty?
      error("Usage: turbot apps:rename NEWNAME\nMust specify NEWNAME to rename.")
    end
    validate_arguments!
  ...
  #Example:
  #
  # $ turbot apps:destroy -a example --confirm example
  ...
  def destroy
    ...
    unless @app
      error("Usage: turbot apps:destroy --app APP\nMust specify APP to destroy.")
    end
