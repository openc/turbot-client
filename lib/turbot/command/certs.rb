...
#
class Turbot::Command::Certs < Turbot::Command::Base
  SSL_DOCTOR = Excon.new(ENV["SSL_DOCTOR_URL"] || "https://ssl-doctor.turbotapp.com/")
  ...
  #
  def index
    endpoints = turbot.ssl_endpoint_list(app)

    if endpoints.empty?
      display "#{app} has no SSL Endpoints."
      display "Use `turbot certs:add CRT KEY` to add one."
    else
  ...
  def update
    ...
    endpoint = action("Updating SSL Endpoint #{cname} for #{app}") { turbot.ssl_endpoint_update(app, cname, crt, key) }
    display_warnings(endpoint)
    display "Updated certificate details:"
    display_certificate_info(endpoint)
  rescue UsageError
    fail("Usage: turbot certs:update CRT KEY\nMust specify CRT and KEY to update cert.")
  end
  ...
  def info
    ...
    endpoint = action("Fetching SSL Endpoint #{cname} info for #{app}") do
      turbot.ssl_endpoint_info(app, cname)
    end
  ...
  def remove
    ...
    action("Removing SSL Endpoint #{cname} from #{app}") do
      turbot.ssl_endpoint_remove(app, cname)
    end
  ...
  def rollback
    ...
    endpoint = action("Rolling back SSL Endpoint #{cname} for #{app}") do
      turbot.ssl_endpoint_rollback(app, cname)
    end
  ...
  def current_endpoint
    endpoint = turbot.ssl_endpoint_list(app).first || error("#{app} has no SSL Endpoints.")
    endpoint["cname"]
  end
  ...
  def read_crt_and_key_through_ssl_doctor(action_text = nil)
    ...
    Turbot::OkJson.decode(crt_and_key).values_at("pem", "key")
  end
