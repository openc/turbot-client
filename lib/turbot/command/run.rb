...
#
class Turbot::Command::Run < Turbot::Command::Base
  ...
  def index
    command = args.join(" ")
    error("Usage: turbot run COMMAND") if command.empty?
  ...
  def detached
    ...
    if options[:tail]
      ...
      display("Use `turbot logs -p #{process_data['process']}` to view the output.")
    end
  end
  ...
  #
  def rake
    deprecate("`turbot #{current_command}` has been deprecated. Please use `turbot run rake` instead.")
  ...
  #
  # NOTE: For Cedar apps, use `turbot run console`
  #
  #Examples:
  #
  # $ turbot console
  # Ruby console for example.turbot.com
  # >>
  #
  def console
    puts "`turbot #{current_command}` has been removed. Please use: `turbot run` instead."
    puts "For more information, please see:"
    puts " * https://devcenter.turbot.com/articles/one-off-dynos"
    puts " * https://devcenter.turbot.com/articles/rails3#console"
    puts " * https://devcenter.turbot.com/articles/console-bamboo"
  end
...
protected
  ...
  def console_history_dir
    FileUtils.mkdir_p(path = "#{home_directory}/.turbot/console_history")
    path
  end

  def console_session(app)
    turbot.console(app) do |console|
      console_history_read(app)

      display "Ruby console for #{app}.#{turbot.host}"
