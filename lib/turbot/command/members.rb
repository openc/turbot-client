...
#
class Turbot::Command::Members < Turbot::Command::Base
  ...
  def add
    unless member = shift_argument
      error("Usage: turbot members:add EMAIL\nMust specify EMAIL to add.")
    end
  ...
  def set
    unless member = shift_argument
      error("Usage: turbot members:set EMAIL\nMust specify EMAIL to update.")
    end
  ...
  def remove
    unless member = shift_argument
      error("Usage: turbot members:remove EMAIL\nMust specify EMAIL to remove.")
    end
