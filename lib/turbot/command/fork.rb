require "turbot/client/cisaurus"
...
module Turbot::Command
  ...
  class Fork < Base
    ...
    # fork [NEWNAME]
    #
    # Fork an existing app -- copy config vars and Turbot Postgres data, and re-provision add-ons to a new app.
    ...
    def index
      ...
      if from == to
        raise Turbot::Command::CommandFailed.new("Cannot fork to the same app.")
      end
      ...
      raise if e.is_a?(Turbot::Command::CommandFailed)
  ...
  private
    ...
    def cisaurus_client
      cisaurus_url = ENV["CISAURUS_HOST"] || "https://cisaurus.turbotapp.com"
      @cisaurus_client ||= Turbot::Client::Cisaurus.new(cisaurus_url)
    end
    ...
    def migrate_db(from_addon, from, to_addon, to)
      ...
      action("Transferring database (this can take some time)") do
        ...
        pgb = Turbot::Client::Pgbackups.new(from_config["PGBACKUPS_URL"])
    ...
    def pg_api
      ...
      RestClient::Resource.new "https://#{host}/client/v11/databases", Turbot::Auth.user, Turbot::Auth.password
    end
