...
# manage turbot-postgresql databases
#
class Turbot::Command::Pg < Turbot::Command::Base
  ...
  def index
    ...
    if hpg_databases_with_info.empty?
      display("#{app} has no turbot-postgresql databases.")
    else
  ...
  def promote
    unless db = shift_argument
      error("Usage: turbot pg:promote DATABASE\nMust specify DATABASE to promote.")
    end
    validate_arguments!
  ...
  def psql
    ...
    begin
      ...
      output_with_bang "For help installing psql, see http://devcenter.turbot.com/articles/local-postgresql"
      abort
    end
  end
  ...
  def reset
    unless db = shift_argument
      error("Usage: turbot pg:reset DATABASE\nMust specify DATABASE to reset.")
    end
    validate_arguments!
  ...
  def unfollow
    unless db = shift_argument
      error("Usage: turbot pg:unfollow REPLICA\nMust specify REPLICA to unfollow.")
    end
    validate_arguments!
  ...
  def push
    ...
    unless [remote, local].all?
      Turbot::Command.run(current_command, ['--help'])
      exit(1)
    end
    if local =~ %r(://)
...
private
  ...
  def hpg_client(attachment)
    Turbot::Client::TurbotPostgresql.new(attachment)
  end
  ...
  def exec_sql_on_uri(sql,uri)
    begin
      ...
      output_with_bang "The local psql command could not be located"
      output_with_bang "For help installing psql, see https://devcenter.turbot.com/articles/turbot-postgresql#local-setup"
      abort
    end
  end
