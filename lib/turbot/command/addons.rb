  ...
  class Addons < Base
    ...
    def add
      configure_addon('Adding') do |addon, config|
        turbot.install_addon(app, addon, config)
      end
    end
    ...
    def upgrade
      configure_addon('Upgrading to') do |addon, config|
        turbot.upgrade_addon(app, addon, config)
      end
    end
    ...
    def remove
      ...
      args.each do |name|
        ...
        action("Removing #{name} on #{app}") do
          messages = addon_run { turbot.uninstall_addon(app, name, :confirm => app) }
        end
    ...
    def docs
      unless addon = shift_argument
        error("Usage: turbot addons:docs ADDON\nMust specify ADDON to open docs for.")
      end
      validate_arguments!
    ...
    def open
      unless addon = shift_argument
        error("Usage: turbot addons:open ADDON\nMust specify ADDON to open.")
      end
      validate_arguments!
    ...
    def addon_docs_url(addon)
      "https://devcenter.#{turbot.host}/articles/#{addon.split(':').first}"
    end

    def app_addon_url(addon)
      "https://addons-sso.turbot.com/apps/#{app}/addons/#{addon}"
    end
    ...
    def addon_run
      ...
      if response
        ...
        rescue Turbot::API::Errors::Error
          release = nil
        end
      end
      ...
      error Turbot::Command.extract_error(e.http_body) {
    ...
    def configure_addon(label, &install_or_upgrade)
      ...
      # For Turbot Postgres, if no plan is specified with fork/follow/rollback,
      # default to the plan of the current postgresql plan
      if addon_name =~ /turbot-postgresql/ then
      ...
      display("Use `turbot addons:docs #{addon_name}` to view documentation.")
    end
