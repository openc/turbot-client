require "spec_helper"
require "turbot/command/ps"

describe Turbot::Command::Ps do
  ...
  context("cedar") do
    ...
    it "ps:dynos errors out on cedar apps" do
      lambda { execute("ps:dynos") }.should raise_error(Turbot::Command::CommandFailed, "For Cedar apps, use `turbot ps`")
    end

    it "ps:workers errors out on cedar apps" do
      lambda { execute("ps:workers") }.should raise_error(Turbot::Command::CommandFailed, "For Cedar apps, use `turbot ps`")
    end
    ...
    describe "ps" do
      ...
      it "displays processes" do
        ...
        Turbot::Command::Ps.any_instance.should_receive(:time_ago).exactly(10).times.and_return("2012/09/11 12:34:56 (~ 0s ago)")
...
STDOUT
      ...
      it "displays one-off processes" do
        ...
        Turbot::Command::Ps.any_instance.should_receive(:time_ago).twice.and_return('2012/09/11 12:34:56 (~ 0s ago)')
        stderr, stdout = execute("ps")
        stderr.should == ""
        stdout.should == <<-STDOUT
...
STDOUT
      ...
      it "displays multiple sizes for one-offs" do
        ...
        Turbot::Command::Ps.any_instance.should_receive(:time_ago).exactly(4).times.and_return("2012/09/11 12:34:56 (~ 0s ago)")
        stderr, stdout = execute("ps")
        stderr.should == ""
        stdout.should == <<-STDOUT
=== run: one-off processes
  ...
  context("non-cedar") do
    ...
    describe "ps:dynos" do
      ...
      it "displays the current number of dynos" do
        ...
        stderr.should == ""
        stdout.should == <<-STDOUT
~ `turbot ps:dynos QTY` has been deprecated and replaced with `turbot ps:scale dynos=QTY`
...
STDOUT
    ...
    describe "ps:workers" do
      ...
      it "displays the current number of workers" do
        ...
        stderr.should == ""
        stdout.should == <<-STDOUT
~ `turbot ps:workers QTY` has been deprecated and replaced with `turbot ps:scale workers=QTY`
