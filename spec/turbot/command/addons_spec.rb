        ...
        Excon.stub(
          ...
          },
          {
            :body   => Turbot::OkJson.encode([
              ...
              { 'attachment_name' => 'HEROKU_POSTGRESQL_RED', 'configured' => true, 'name' => 'turbot-postgresql:ronin' },
...
deployhooks:http
turbot-postgresql:ronin  HEROKU_POSTGRESQL_RED

=== example Add-ons to Configure
deployhooks:email  https://addons-sso.turbot.com/apps/example/addons/deployhooks:email
    ...
    describe "list" do
      ...
      it "sends region option to the server" do
        stub_request(:get, %r{/addons\?region=eu$}).
          to_return(:body => Turbot::OkJson.encode([]))
...
STDOUT
    ...
    describe 'v1-style command line params' do
      it "understands foo=baz" do
        ...
        @addons.turbot.should_receive(:install_addon).with('example', 'my_addon', { 'foo' => 'baz' })
        @addons.add
      end
      ...
      it "gives a deprecation notice with an example" do
        stub_request(:post, %r{apps/example/addons/my_addon$}).
          ...
          to_return(:body => Turbot::OkJson.encode({ 'price' => 'free' }))
        Excon.stub(
          {
            :expects => 200,
            :method => :get,
          ...
          {
            :body   => Turbot::OkJson.encode({ 'name' => 'v99' }),
            :status => 200,
          }
        )
...
Use `turbot addons:docs my_addon` to view documentation.
STDOUT
        Excon.stubs.shift
      end
    end
    ...
    describe 'unix-style command line params' do
      ...
      it "treats lone switches as true" do
        ...
        @addons.turbot.should_receive(:install_addon).with('example', 'my_addon', { 'foo' => true })
        @addons.add
      end
      ...
      it "works with many config vars" do
        ...
        @addons.turbot.should_receive(:install_addon).with('example', 'my_addon', { 'foo' => 'baz', 'bar' => 'yes', 'baz' => 'foo', 'bab' => true, 'bob' => true })
        @addons.add
      end
    ...
    describe "mixed options" do
      it "understands foo=bar and --baz=bar on the same line" do
        ...
        @addons.turbot.should_receive(:install_addon).with('example', 'my_addon', { 'foo' => 'baz', 'baz' => 'bar', 'bar' => true, 'bob' => true })
        @addons.add
      end
    ...
    describe "fork, follow, and rollback switches" do
      ...
      it "should translate --fork, --follow, and --rollback" do
        %w{fork follow rollback}.each do |switch|
          Turbot::Helpers::TurbotPostgresql::Resolver.any_instance.stub(:app_config_vars).and_return({})
          Turbot::Helpers::TurbotPostgresql::Resolver.any_instance.stub(:app_attachments).and_return([Turbot::Helpers::TurbotPostgresql::Attachment.new({
              'app' => {'name' => 'sushi'},
          ...
          @addons.stub!(:args).and_return("turbot-postgresql --#{switch} HEROKU_POSTGRESQL_RED".split)
          @addons.turbot.should_receive(:install_addon).with('example', 'turbot-postgresql:ronin', {switch => 'postgres://red_url'})
          @addons.add
        end
      end
      ...
      it "should NOT translate --fork and --follow if passed in a full postgres url even if there are no databases" do
        %w{fork follow}.each do |switch|
          ...
          @addons.stub!(:args).and_return("turbot-postgresql:ronin --#{switch} postgres://foo:yeah@awesome.com:234/bestdb".split)
          @addons.turbot.should_receive(:install_addon).with('example', 'turbot-postgresql:ronin', {switch => 'postgres://foo:yeah@awesome.com:234/bestdb'})
          @addons.add
        end
      end
      ...
      it "should fail if fork / follow across applications and no plan is specified" do
        %w{fork follow}.each do |switch|
          @addons.stub!(:app_config_vars).and_return({})
          @addons.stub!(:app_attachments).and_return([])
          @addons.stub!(:args).and_return("turbot-postgresql --#{switch} postgres://foo:yeah@awesome.com:234/bestdb".split)
          lambda { @addons.add }.should raise_error(CommandFailed)
        end
      end
    end
    ...
    describe 'adding' do
      ...
      it "adds an addon" do
        @addons.stub!(:args).and_return(%w(my_addon))
        @addons.turbot.should_receive(:install_addon).with('example', 'my_addon', {})
        @addons.add
      end
...
OUTPUT
      ...
      it "displays an error with unexpected options" do
        Turbot::Command.should_receive(:error).with("Unexpected arguments: bar")
    ...
    describe 'upgrading' do
      ...
      it "upgrades an addon" do
        @addons.stub!(:args).and_return(%w(my_addon))
        @addons.turbot.should_receive(:upgrade_addon).with('example', 'my_addon', {})
        @addons.upgrade
      end

      it "upgrade an addon with config vars" do
        @addons.stub!(:args).and_return(%w(my_addon --foo=baz))
        @addons.turbot.should_receive(:upgrade_addon).with('example', 'my_addon', { 'foo' => 'baz' })
        @addons.upgrade
      end
...
OUTPUT
    ...
    it "does not remove addons with no confirm" do
      ...
      @addons.turbot.should_not_receive(:uninstall_addon)
      @addons.remove
    end
    ...
    it "removes addons after prompting for confirmation" do
      ...
      @addons.turbot.should_receive(:uninstall_addon).with('example', 'addon1', :confirm => "example")
      @addons.remove
    end
 ...
 !    Usage: turbot addons:docs ADDON
...
STDERR
      ...
      it "opens the addon if only one matches" do
        require("launchy")
        Launchy.should_receive(:open).with("https://devcenter.turbot.com/articles/redistogo").and_return(Thread.new {})
...
STDERR
      ...
      it "complains if no such addon exists" do
        ...
        stderr.should == <<-STDERR
 !    `unknown` is not a turbot add-on.
 !    See `turbot addons:list` for all available addons.
STDERR
        stdout.should == ''
      end
      ...
      it "suggests alternatives if addon has typo" do
        ...
        stderr.should == <<-STDERR
 !    `redisgoto` is not a turbot add-on.
...
STDOUT
    ...
    describe "opening an addon" do
      ...
      it "displays usage when no argument is specified" do
        ...
        stderr.should == <<-STDERR
 !    Usage: turbot addons:open ADDON
...
STDERR
      ...
      it "opens the addon if only one matches" do
        ...
        require("launchy")
        Launchy.should_receive(:open).with("https://addons-sso.turbot.com/apps/example/addons/redistogo:nano").and_return(Thread.new {})
