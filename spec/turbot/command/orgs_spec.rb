...
STDOUT
      ...
      it "lists orgs with roles that the user belongs to" do
        Excon.stub({ :method => :get, :path   => '/v1/user/info' },
          {
            :body   => Turbot::OkJson.encode({"organizations" => [{"organization_name" => "test-org", "role" => "collaborator"}, {"organization_name" => "test-org2", "role" => "admin"}], "user" => {}}),
            :status => 200
          }
        )
...
STDOUT
      ...
      it "labels a user's default organization" do
        Excon.stub({ :method => :get, :path   => '/v1/user/info' },
          {
            :body   => Turbot::OkJson.encode({"organizations" => [{"organization_name" => "test-org", "role" => "collaborator"}, {"organization_name" => "test-org2", "role" => "admin"}], "user" => {"default_organization" => "test-org2"}}),
            :status => 200
          }
        )
...
STDOUT
      ...
      context "when no target is specified" do
        it "displays the default organization when present" do
          Excon.stub({ :method => :get, :path   => '/v1/user/info' },
            {
              :body   => Turbot::OkJson.encode({"user" => {"default_organization" => "test-org"}}),
              :status => 200
            }
          )
...
STDOUT
    ...
    context(:open) do
      before(:each) do
        require("launchy")
        ::Launchy.should_receive(:open).with("https://dashboard.turbot.com/orgs/test-org/apps").once.and_return("")
      end
...
STDOUT
      ...
      it "opens the default org" do
        Excon.stub({ :method => :get, :path   => '/v1/user/info' },
          {
            :body   => Turbot::OkJson.encode({"organizations" => [{"organization_name" => "test-org"}], "user" => {"default_organization" => "test-org"}}),
            :status => 200
          }
        )
