...
Issuer:         /C=US/ST=California/L=San Francisco/O=Turbot by Salesforce/CN=secure.example.org
Starts At:      2012-08-01 21:34 UTC
Subject:        /C=US/ST=California/L=San Francisco/O=Turbot by Salesforce/CN=secure.example.org
SSL certificate is self signed.
      CERTIFICATE_DETAILS
    }

    let(:endpoint) {
      { 'cname'          => 'tokyo-1050.turbotssl.com',
        'ssl_cert' => {
          'ca_signed?'   => false,
    ...
    let(:endpoint2) {
      { 'cname'          => 'akita-7777.turbotssl.com',
        'ssl_cert' => {
          'ca_signed?'   => true,
          'cert_domains' => [ 'turbot.com' ],
          'starts_at'    => "2012-08-01 21:34:23 UTC",
          'expires_at'   => "2013-08-01 21:34:23 UTC",
          'issuer'       => '/C=US/ST=California/L=San Francisco/O=Turbot by Salesforce/CN=secure.example.org',
          'subject'      => '/C=US/ST=California/L=San Francisco/O=Turbot by Salesforce/CN=secure.example.org',
...
------------------------  --------------  --------------------  -------
tokyo-1050.turbotssl.com  example.org     2013-08-01 21:34 UTC  False
akita-7777.turbotssl.com  turbot.com      2013-08-01 21:34 UTC  True
STDOUT
      end
...
Use `turbot certs:add CRT KEY` to add one.
        STDOUT
      end
    end
...
example now served by tokyo-1050.turbotssl.com
Certificate details:
#{certificate_details}
        STDOUT
      end
    ...
    describe "certs:info" do
      it "shows certificate details" do
        ...
        stub_core.ssl_endpoint_info('example', 'tokyo-1050.turbotssl.com').returns(endpoint)

        stderr, stdout = execute("certs:info")
        stdout.should == <<-STDOUT
Fetching SSL Endpoint tokyo-1050.turbotssl.com info for example... done
Certificate details:
#{certificate_details}
        STDOUT
      end
 ...
 !    example has no SSL Endpoints.
    ...
    describe "certs:remove" do
      it "removes an endpoint" do
        stub_core.ssl_endpoint_list("example").returns([endpoint])
        stub_core.ssl_endpoint_remove('example', 'tokyo-1050.turbotssl.com').returns(endpoint)

        stderr, stdout = execute("certs:remove --confirm example")
        stdout.should include "Removing SSL Endpoint tokyo-1050.turbotssl.com from example..."
      ...
      it "requires confirmation" do
        ...
        stdout.should include "This command will remove the endpoint tokyo-1050.turbotssl.com from example."
      end
 ...
 !    example has no SSL Endpoints.
    ...
    describe "certs:update" do
      ...
      it "updates an endpoint" do
        stub_core.ssl_endpoint_list("example").returns([endpoint])
        stub_core.ssl_endpoint_update('example', 'tokyo-1050.turbotssl.com', 'pem content', 'key content').returns(endpoint)

        stderr, stdout = execute("certs:update --confirm example --bypass pem_file key_file")
        stdout.should == <<-STDOUT
Updating SSL Endpoint tokyo-1050.turbotssl.com for example... done
...
#{certificate_details}
      ...
      it "requires confirmation" do
        ...
        stdout.should include "WARNING: Potentially Destructive Action"
        stdout.should include "This command will change the certificate of endpoint tokyo-1050.turbotssl.com on example."
      end
 ...
 !    example has no SSL Endpoints.
    ...
    describe "certs:rollback" do
      it "performs a rollback on an endpoint" do
        stub_core.ssl_endpoint_list("example").returns([endpoint])
        stub_core.ssl_endpoint_rollback('example', 'tokyo-1050.turbotssl.com').returns(endpoint)

        stderr, stdout = execute("certs:rollback --confirm example")
        stdout.should == <<-STDOUT
Rolling back SSL Endpoint tokyo-1050.turbotssl.com for example... done
...
#{certificate_details}
      ...
      it "requires confirmation" do
        ...
        stdout.should include "WARNING: Potentially Destructive Action"
        stdout.should include "This command will rollback the certificate of endpoint tokyo-1050.turbotssl.com on example."
      end
